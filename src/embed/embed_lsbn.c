...
status_code embed_lsbn(unsigned char n, char* in_file_path, char* p_file_path, char* out_file_path) {
    ...
    color * current_color = NULL; // Only for LSBI
    ...
    if ((exit_code = embed_number_lsbni(n, p_file, out_file, in_file_size, false, current_color)) != SUCCESS) {
        goto finally;
    }

    // Embed in_file content in out_file
    if ((exit_code = embed_file_lsbni(n, p_file, out_file, in_file, false, current_color)) != SUCCESS) {
        goto finally;
    }

    // Embed in_file extension in out_file
    if ((exit_code = embed_bytes_lsbni(n, p_file, out_file, (uint8_t*)extension, in_extension_size + 1, false, current_color)) != SUCCESS) {
        goto finally;
    }
